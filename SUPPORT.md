# Support

ChangelogGenie is currently an early prototype.

## Questions and feedback

Use GitHub Issues for:

- bug reports
- setup problems
- confusing documentation
- feedback on generated changelog output
- suggestions for future GitHub Action or GitHub App support

## Security or privacy concerns

Do not post secrets, API keys, private repository content, or sensitive logs in public issues.

If you need to report a sensitive concern, open a minimal public issue asking for a private contact path, without including sensitive details.

## Current support status

This repository is maintained as an early experimental project. Response times are not guaranteed.
